...
class RotateGrating(StimulusBuilder):

    def __init__(self, parent, shared, trigger_pixel, T = 10.0, rotation = 0.0, wave_length = 30.0 , velocity = 10.0, mode = 'sq', min_intensity = 0, max_intensity = 255):

        arg = (T, rotation, wave_length, velocity, mode, min_intensity, max_intensity)
        StimulusBuilder.__init__(self, parent, shared, trigger_pixel, arg)

        # define parameter names
        self.parameter_names = ['stimulus duration [s]', 'rotation [deg]', 'wavelength [deg]', 'velocity [deg/s]', 'mode [sq/sin]', 'min_intensity', 'max_intensity']
    ...
    def increase_rot(self):
        self.shared.rotation += 1
        print("rotation: " + str(self.shared.rotation))

    def decrease_rot(self):
        self.shared.rotation -= 1
        print("rotation: " + str(self.shared.rotation))
    ...
    def run(self, time):
        ...
        self.cylinder.setTexRotate(pcore.TextureStage.getDefault(), -self.shared.rotation)
